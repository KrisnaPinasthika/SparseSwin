...
import torch
import math
from src.SparseSwinEncoder import SparseSwinEncoder 
...
class SparseSwinDet(nn.Module):
    def __init__(self, sparseswin_type, num_anchors=9, num_classes=80, compound_coef=0, 
                    input_resolution=512, device='cuda'):
        super(SparseSwinDet, self).__init__()
        sparseswin_type = sparseswin_type.lower()
        if sparseswin_type == 'tiny': 
            ltoken_num, ltoken_dims = 49, 512 
        elif sparseswin_type == 'small': 
            ltoken_num, ltoken_dims = 64, 768 
        elif sparseswin_type == 'base': 
            ltoken_num, ltoken_dims = 81, 1024 
        
        self.backbone_net = SparseSwinEncoder(
                                c_dim_3rd=384, 
                                hw_size_3rd=input_resolution//16, 
        ...
        self.conv3 = nn.Conv2d(192, self.num_channels, kernel_size=1, stride=1, padding=0)
        self.conv4 = nn.Conv2d(384, self.num_channels, kernel_size=1, stride=1, padding=0)
        self.conv5 = nn.Conv2d(ltoken_dims, self.num_channels, kernel_size=1, stride=1, padding=0)
        self.conv6 = nn.Conv2d(ltoken_dims, self.num_channels, kernel_size=3, stride=2, padding=1)
    ...
    def forward(self, inputs):
        ...
        c3, c4, c5 = self.backbone_net(img_batch)[1:]
        p3 = self.conv3(c3)
        p4 = self.conv4(c4)
        p5 = self.conv5(c5)
        p6 = self.conv6(c5)
        ...
        # print(f"classification : {classification.shape}, regression : {regression.shape}, anchors : {anchors.shape}, annotations : {annotations.shape}")
...
if __name__ == '__main__':
    ...
    sample_img = torch.randn((1, 3, 512, 512)).to(device)
